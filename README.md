...
This repository contains a GPU-accelerated Dask workflow for the POD-based computation of multiscale triadic interactions from large flow datasets. It provides a reduced public test case, example outputs, plotting scripts, and submission scripts illustrating how the workflow can be deployed on an HPC system.

The public version of the workflow is distributed in reduced form. It is intended to demonstrate the complete computational pipeline in a lightweight and reproducible way, while the corresponding large production dataset remains outside the scope of this repository.
...
- `FLOW/`: input-data directory for the workflow. It contains the mesh and the sequence file used by the reduced public test case. The full set of raw flow snapshots is not stored directly in this GitHub repository because of its size. The reduced public input dataset can be retrieved from Zenodo.
- `STATS/`: output-data directory for the workflow. It contains example outputs from the POD step together with example triadic output. Some large files, such as mean fields and Parquet outputs stored in directory format, may be distributed through Zenodo rather than directly through GitHub.
...
1. POD/SVD computation from the reduced flow snapshots.
...
The `FLOW/` directory mirrors the input-data structure used by the workflow. The mesh and the sequence file required by the reduced case are provided in the repository. The raw flow snapshots themselves are not stored directly in GitHub because of their size and are instead distributed separately through Zenodo.
...
The `Processing/` directory contains example plots generated from the reduced public test case, including:
...
A Zenodo DOI for the archived public release will be added here after the first release is deposited.
